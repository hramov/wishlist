...
export async function deleteWishCb(instance: TelegramBot, cb: TelegramBot.CallbackQuery) {
...
export async function buyWishCb(instance: TelegramBot, cb: TelegramBot.CallbackQuery) {
    await instance.answerCallbackQuery(cb.id);
    const result = await new Wish(null).markWishAsGifted(Number(cb.data.split(" ")[1]), cb.from.id);
    if (result != null) {
